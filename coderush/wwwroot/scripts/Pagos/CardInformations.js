(function (document, Heartland) {
    // Cree un nuevo objeto`HPS` con la configuración necesaria
    var hps = new Heartland.HPS({
        publicKey: 'pkapi_cert_P6dRqs1LzfWJ6HgGVZ',
        type: 'iframe',

        // Configure los campos del iframe para decirle a la biblioteca dónde
        // el iframe debe insertarse en el DOM y algunos
        // opciones básicas
        fields: {
            cardNumber: {
        ...
        // Colección de CSS para inyectar en los iframes.
        // Estas propiedades pueden coincidir con los estilos del sitio
        // para crear una experiencia perfecta.
        style: {
            'input': {
                'background': '#fff',
                'border': '1px solid',
        ...
        // Devolución de llamada cuando se recibe un token del servicio
        onTokenSuccess: function (resp) {
            // crea un campo y agrega al formulario
            $("<input>").attr({ type: "hidden", id: "Token_value", name: "Token_value", value: resp.token_value }).appendTo("#iframestoken");
            // desenlaza el controlador de eventos
            $("#payment_form").unbind('submit');
            // enviar el formulario
            $("#payment_form").submit();
        },

        // Devolución de llamada cuando se recibe un error del servicio
        onTokenError: function (resp) {
            alert('There was an error: ' + resp.error.message);
           
        },

        // Devolución de llamada cuando se dispara un evento dentro de un iFrame
        onEvent: function (ev) {
            console.log(ev);
        }
    });
    ...
    // Adjunte un controlador para interrumpir el envío del formulario
    Heartland.Events.addHandler(document.getElementById('iframes'), 'submit', function (e) {

        // Evita que el formulario continúe hasta la dirección `action`
        e.preventDefault();
